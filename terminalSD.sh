...
qemu-system-arm -m 1024 -boot order=ncd -machine vexpress-a9,accel=tcg \
 -cpu cortex-a9 -kernel $kernel -append "root=/dev/mmcblk0p2 mem=1024M devtmpfs.mount=0 rw console=ttyAMA0 console=ttyS0" \
 ...
 -netdev user,id=user.0 -device virtio-net-device,netdev=user.0 -initrd $init \
